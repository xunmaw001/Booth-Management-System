        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">摊位信息管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">摊位信息列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">摊位信息列表</h3>
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            row.appendChild(quyuTypesCell);
                ...
                //详情信息
            var taanweiContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.taanweiContent && item.taanweiContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.taanweiContent.indexOf('<img');
                var endIndex = item.taanweiContent.indexOf('>');
                    taanweiContentCell.innerHTML = item.taanweiContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.taanweiContent != null && item.taanweiContent != "" && item.taanweiContent.length >= 11) {
                        taanweiContentCell.innerHTML = item.taanweiContent.substring(0, 10) + "...";
                } else {
                        taanweiContentCell.innerHTML = item.taanweiContent;
                }
            }
            row.appendChild(taanweiContentCell);
            ...
            debugger
            if(item.zhuangtaiValue == "空闲"){
                //详情按钮
                var detailBtn = document.createElement('button');
                var detailAttr = "shenqing(" + item.id + ')';
                detailBtn.setAttribute("type", "button");
                detailBtn.setAttribute("class", "btn btn-primary btn-sm 申请");
                detailBtn.setAttribute("onclick", detailAttr);
                detailBtn.innerHTML = "申请摊位";
                btnGroup.appendChild(detailBtn);
            }
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("taanwei/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        function shenqing(id) {
            var mymessage = confirm("真的要申请这个摊位吗？");
            if (mymessage == true) {
                http("taanwei/shenqing?ids="+id, "get", {}, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('操作成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addtaanwei", "addtaanwei");
            window.location.href = "add-or-update.jsp"
        }
